...
class AppRepository(private val dao: AppDao, private val context: android.content.Context? = null) {
    ...
    private fun getOrCreateGuestUserCode(): String {
        if (context != null) {
            val sharedPrefs = context.getSharedPreferences("soyaagaci_guest_prefs", android.content.Context.MODE_PRIVATE)
            var savedCode = sharedPrefs.getString("guest_user_code", "") ?: ""
            if (savedCode.isBlank() || savedCode.length != 6) {
                val charPool: List<Char> = ('A'..'Z') + ('0'..'9')
                savedCode = (1..6)
                    .map { kotlin.random.Random.nextInt(0, charPool.size) }
                    .map(charPool::get)
                    .joinToString("")
                sharedPrefs.edit().putString("guest_user_code", savedCode).apply()
            }
            return savedCode
        }
        return "8F3A21"
    ...
    suspend fun initializeDefaultDataIfNeeded() {
        ...
        val fixedCode = if (!currentUid.isNullOrBlank()) {
            generateDeterministicUserCode(currentUid)
        } else if (currentProfile?.userCode?.isNotBlank() == true && currentProfile.userCode.length == 6 && currentProfile.userCode != "8F3A21") {
            currentProfile.userCode
        } else {
            getOrCreateGuestUserCode()
        }
        ...
        } else if (currentProfile.userCode != fixedCode) {
            dao.insertOrUpdateProfile(currentProfile.copy(userCode = fixedCode))
        }
    ...
    suspend fun syncFromFirestoreIfLoggedIn() {
        ...
        val userCode = generateDeterministicUserCode(uid) // Always permanent & bound to Firebase Auth UID
        withContext(Dispatchers.IO) {
            try {
                // Fetch User Profile from Firestore
                val userDocSnap = firestore.collection("users").document(uid).get().await()

                // Always sync deterministic userCode to Firestore
                firestore.collection("users").document(uid).set(
                    mapOf("userCode" to userCode, "uid" to uid), SetOptions.merge()
                ).await()
                ...
                for (doc in friendsSnap.documents) {
                    ...
                    val friendAvatar = doc.getString("avatarUri") ?: ""
                    val relationship = doc.getString("relationship") ?: "Arkadaş"
                    val status = doc.getString("connectionStatus") ?: doc.getString("status") ?: "accepted"
                    ...
                    if (friendName.isNotBlank()) {
                        ...
                        if (existing == null) {
                            ...
                            val updated = existing.copy(
                                avatarUri = if (friendAvatar.isNotBlank()) friendAvatar else existing.avatarUri,
                                connectionStatus = status
                            )
                            dao.updateFamilyMember(updated)
                        }
                    }
                }
    ...
    suspend fun updateProfile(profile: UserProfileEntity) {
        ...
        val finalCode = if (!currentUid.isNullOrBlank()) {
            generateDeterministicUserCode(currentUid)
        } else if (profile.userCode.isNotBlank() && profile.userCode.length == 6 && profile.userCode != "8F3A21") {
            profile.userCode
        } else {
            getOrCreateGuestUserCode()
        }
            ...
            try {
                firestore.collection("users").document(uid).set(
                    mapOf(
                        ...
                        "avatarUri" to finalProfile.avatarUri,
    ...
    suspend fun addFriendTwoWayByCode(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val targetAvatar = targetDoc.getString("avatarUri") ?: ""
                val currentAvatar = currentUserDocSnap.getString("avatarUri") ?: ""
                ...
                val dataA = mapOf(
                    ...
                    "avatarUri" to targetAvatar,
                    "connectionStatus" to "pending_outgoing",
                ...
                val dataB = mapOf(
                    ...
                    "relationship" to relationship,
                    "avatarUri" to currentAvatar,
                    "connectionStatus" to "pending_incoming",
                    "addedAt" to System.currentTimeMillis()
                )
                ...
                val newMember = existing?.copy(
                    avatarUri = if (targetAvatar.isNotBlank()) targetAvatar else existing.avatarUri,
                    connectionStatus = "pending_outgoing"
                ) ?: FamilyMemberEntity(
                    userCode = cleanCode,
                    name = targetName,
                    relationship = relationship,
                    avatarUri = targetAvatar,
                    connectionStatus = "pending_outgoing",
                    notes = notes.ifBlank { "Çift taraflı Firestore arkadaşı (UID: $targetUid)" },
                    isDeceased = false
                )
                val id = if (existing == null) dao.insertFamilyMember(newMember) else {
                    dao.updateFamilyMember(newMember)
                    existing.id
                }
    ...
    suspend fun updateConnectionStatus(member: FamilyMemberEntity, newStatus: String) {
        val updated = member.copy(connectionStatus = newStatus)
        dao.updateFamilyMember(updated)
        ...
        withContext(Dispatchers.IO) {
            try {
                if (member.userCode.isNotBlank()) {
                    val querySnap = firestore.collection("users")
                        .whereEqualTo("userCode", member.userCode)
                        .get()
                        .await()
                    if (!querySnap.isEmpty) {
                        val targetUid = querySnap.documents[0].getString("uid") ?: querySnap.documents[0].id

                        firestore.collection("users").document(currentUid)
                            .collection("friends").document(targetUid)
                            .set(mapOf("connectionStatus" to newStatus, "status" to newStatus), SetOptions.merge())
                ...
                Log.e("AppRepository", "updateConnectionStatus Firestore error: ${e.message}")
            }
        }
    }

    suspend fun clearChatHistoryForMember(memberId: Long) {
        dao.deleteChatMessagesForMember(memberId)
    }
