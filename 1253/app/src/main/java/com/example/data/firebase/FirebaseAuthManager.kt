...
class FirebaseAuthManager(private val context: Context) {
    ...
    fun signInWithFacebook(
        ...
        try {
            val providerBuilder = OAuthProvider.newBuilder("facebook.com")
            providerBuilder.scopes = listOf("email", "public_profile")

            val pendingResultTask = try { auth.pendingAuthResult } catch (e: Throwable) { null }
            if (pendingResultTask != null) {
                        ...
                        try {
                            ...
                            if (user != null) {
                                val firestoreUser = FirestoreUser(
                                    ...
                                    name = user.displayName ?: "Facebook Kullanıcısı",
                                    email = user.email ?: "",
                                ...
                                onFailure(Exception("Facebook kullanıcı bilgisi alınamadı."))
                            }
                        } catch (e: Throwable) {
                            onFailure(Exception(e.message ?: "Facebook girişi hatası"))
                        }
                    }
                ...
                val task = try {
                    ...
                    onFailure(Exception("Facebook OAuth başlatılamadı: ${e.message}"))
                    return
                }
            ...
            onFailure(Exception("Facebook ile giriş hatası: ${e.message}"))
        }
    }
    ...
    private suspend fun saveUserToFirestore(user: FirestoreUser): String {
        return try {
            ...
            val userCode = if (user.uid.isNotBlank()) generateDeterministicUserCode(user.uid) else user.userCode.ifBlank { docSnap?.getString("userCode") ?: "" }
    ...
    private fun translateFirebaseError(e: Exception): String {
        ...
        return when {
            ...
            message.contains("provider is not enabled", ignoreCase = true) ||
                    message.contains("CONFIGURATION_NOT_FOUND", ignoreCase = true) ->
                "Firebase Konsolu'nda Facebook Girişi sağlayıcısı henüz aktif edilmemiş. Lütfen Firebase Console -> Authentication -> Sign-in method kısmından Facebook'u aktifleştirin."
